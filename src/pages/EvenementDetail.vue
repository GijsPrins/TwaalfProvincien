...
const isUpcoming = computed(() => event.value && ['interested', 'signed_up'].includes(event.value.status))
const isFinished = computed(() => event.value && ['completed', 'dnf'].includes(event.value.status))
